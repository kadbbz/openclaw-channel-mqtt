...
async function handleInboundMessage(opts: {
  ...
  try {
    ...
    log?.info?.(
      `Inbound MQTT message on ${topic}: ${text.slice(0, 200)}${text.length > 200 ? "..." : ""}`
    );
        ...
        deliver: async (payload: { text?: string; media?: any }, info: { kind: string }) => {
          ...
          if (mqttClient.isConnected()) {
            try {
              ...
              const replyPayload = buildOutboundEnvelope({
                text: replyText,
                kind: info.kind,
              ...
              await mqttClient.publish(outboundTopic, replyPayload, qos as 0 | 1 | 2);
              log?.info?.(
